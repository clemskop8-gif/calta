...
SYNONYMS = {
    ...
    # --- добавлено: частые слова в новостях о транспорте/парке техники,
    # без которых предложение оставалось практически нетронутым ---
    'парк': ['подвижной состав', 'автопарк'],
    'поездов': ['составов', 'подвижного состава'],
    'вагонов': ['единиц подвижного состава', 'вагонов подвижного состава'],
    'приобретено': ['закуплено', 'поставлено', 'получено'],
    'закупит': ['приобретёт', 'закупит партию', 'получит'],
    'обновили': ['модернизировали', 'обновили парк', 'заменили часть'],
    'обновлен': ['модернизирован', 'заменён'],
    'обновлено': ['модернизировано', 'заменено'],
    'последние': ['минувшие', 'прошедшие'],
    'работа': ['деятельность', 'процесс'],
    'продолжается': ['продолжается и сейчас', 'идёт и в настоящее время', 'не останавливается'],
    'новых': ['новейших', 'современных'],
    'новый': ['современный', 'новейший'],
    'новая': ['современная', 'новейшая'],
    'новое': ['современное', 'новейшее'],
    'года': ['лет подряд', 'года подряд'],
    'приобрели': ['закупили', 'получили'],
    'приобретут': ['закупят', 'получат'],
    'до конца': ['к завершению', 'не позднее конца'],
    'общий': ['совокупный', 'суммарный'],
    'общая': ['совокупная', 'суммарная'],
    'общее': ['совокупное', 'суммарное'],
}
...
def restructure_sentence(sentence):
    ...
    # 4) Перестановка частей, разделённых тире: "X — Y." -> "Y — X."
    #    Часто встречается в новостях вида "За N лет обновили парк —
    #    приобретено M вагонов." Без этой перестановки такие предложения
    #    почти дословно совпадали с оригиналом даже после замены синонимов.
    s = swap_dash_clauses(s)

    return s

def swap_dash_clauses(sentence):
    """
    Меняет местами часть до и после тире: "X — Y." -> "Y — X."
    Снижает буквальное совпадение с оригиналом для предложений вида
    "Обновили парк — приобретено 418 вагонов.", где простой заменой
    синонимов число и структуру не изменить.
    """
    s = sentence.strip()
    m = re.match(r'^(.{10,140}?)\s*[—\-–]\s*(.{10,140}?)\.?$', s)
    if m and random.random() < 0.7:
        first, second = m.group(1).strip(), m.group(2).strip()
        second_cap = second[0].upper() + second[1:] if second else second
        first_low = first[0].lower() + first[1:] if first and first[0].isupper() else first
        return f"{second_cap.rstrip('.')} — {first_low.rstrip('.')}."
    return s
...
def _significant_words(text):
    """Значимые слова (от 4 букв, кириллица) для оценки схожести текстов."""
    return set(re.findall(r'[а-яёА-ЯЁ]{4,}', (text or '').lower()))

def overlap_ratio(rewritten, original):
    """
    Доля значимых слов пересказа, которые дословно совпадают со словами
    исходного предложения. Используется как защита от почти дословного
    копирования, когда в предложении не нашлось слов для замены
    синонимами (например, "приобретено 418 новых вагонов").
    """
    orig_words = _significant_words(original)
    new_words = _significant_words(rewritten)
    if not new_words:
        return 0.0
    return len(new_words & orig_words) / len(new_words)

_FACT_TEMPLATES = [
    "По имеющимся данным, ключевой показатель составляет {num}.",
    "Как следует из доступных сведений, речь идёт о {num}.",
    "Соответствующий показатель, по последним данным, находится на уровне {num}.",
    "Согласно приведённым цифрам, показатель достигает {num}.",
]

def build_fact_rewrite(original_sentence):
    """
    Крайний случай: если пересказ предложения после всех преобразований
    всё ещё практически совпадает с оригиналом (нет слов для синонимов,
    нет тире для перестановки), строим фразу из шаблона на основе
    вынутого числового факта — вместо копирования формулировки источника.
    Возвращает None, если в предложении нет чисел (тогда пересказ
    подстраховывается вводным словом в generate_unique_summary).
    """
    numbers = re.findall(
        r'\d[\d\s]*[\.,]?\d*\s*(?:млн|млрд|тыс|процент(?:ов)?|%|тонн|вагонов|км|километров)?',
        original_sentence
    )
    numbers = [n.strip() for n in numbers if n.strip()]
    if numbers:
        return random.choice(_FACT_TEMPLATES).format(num=numbers[0])
    return None
...
def generate_unique_summary(title, original_summary):
    ...
    used_body = False
    for original_s in real_sentences[:3]:
        s = strip_quote_attribution(original_s)
        ...
        # Защита от почти дословного копирования: если после всех
        # преобразований (синонимы, перестройка структуры, перестановка
        # через тире) пересказ всё ещё слишком похож на исходное
        # предложение — значит подходящих слов для замены не нашлось.
        # В этом случае заменяем фразу на фактологический шаблон
        # (если в предложении есть числа) либо, как минимум, меняем
        # начало предложения вводным словом и понижаем регистр остатка.
        if overlap_ratio(s, original_s) > 0.55:
            alt = build_fact_rewrite(original_s)
            if alt:
                s = alt
            else:
                intro = random.choice(['По сути,', 'Фактически,', 'Как отмечается,', 'Судя по всему,'])
                s_low = s[0].lower() + s[1:] if s and s[0].isupper() else s
                s = f"{intro} {s_low}"
